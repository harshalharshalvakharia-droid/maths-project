...
\rhead{\textcolor{navrachana}{\small The Architecture of Intelligence}}
\lhead{\textcolor{navrachana}{\small CME202 \textendash{} Mathematics II}}
...
{\normalsize B.Tech (CE \textbar{} ME \textbar{} EEE) \quad $\boldsymbol{\cdot}$ \quad CME202 \textendash{} Mathematics II}
...
\begin{itemize}[leftmargin=2em, itemsep=3pt]
  ...
  \item \textbf{Weight initialisation:} He initialisation, $w \sim \mathcal{N}\!\left(0,\, \sqrt{2/n_{\mathrm{in}}}\right)$
\end{itemize}
...
\begin{enumerate}[leftmargin=2em, itemsep=3pt]
  ...
  \item Weights initialised from $\mathcal{N}(0,\, 2/n_{\mathrm{in}})$ --- He initialisation, appropriate for sigmoid networks.
...
Computing the Gram matrix elements:
\[
  G_{11} = 0.5^2+0.2^2+0.1^2 = 0.30, \qquad
...
\[
  \lambda_1 \approx 0.892, \qquad \lambda_2 \approx 0.299
\]
\[
  \sigma_1 = \sqrt{\lambda_1} \approx 0.944, \qquad \sigma_2 = \sqrt{\lambda_2} \approx 0.547
\]
\[
  \boxed{\kappa = \frac{\sigma_1}{\sigma_2} \approx 1.73
  \quad\Longrightarrow\quad \text{Well-conditioned --- stable training expected.}}
\]
\end{keyresult}
...
\begin{figure}[H]
  ...
  \caption{Left: $\sigma(z)$ maps any real input to $(0,1)$. Shaded red regions mark saturation zones where $\sigma'(z) \approx 0$. Right: $\sigma'(z)$ peaks at 0.25 when $z=0$ and collapses near saturation. Learning signal is proportional to this derivative --- a saturated neuron contributes essentially nothing to the backward pass.}
  \label{fig:sig}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Left: loss landscape $\mathcal{L}(w)=\frac{1}{2}(\sigma(w)-0.8)^2$ with the gradient descent trajectory from $w_0=-2$. Red dots are iterations; the dashed green line marks $w^*=\ln 4 \approx 1.386$. Right: convergence comparison --- gradient descent takes $\sim\!20$ iterations while Newton--Raphson reaches the same tolerance in $\sim\!5$ steps.}
  \label{fig:loss}
\end{figure}

\subsection{Backpropagation --- The Chain Rule, Layer by Layer}

The network learns by computing $\nabla_{\mathbf{W}^{[l]}}\mathcal{L}$ for every layer and updating $\mathbf{W}^{[l]} \leftarrow \mathbf{W}^{[l]} - \eta\,\nabla_{\mathbf{W}^{[l]}}\mathcal{L}$. The gradient is found by the chain rule applied backwards:
...
\begin{figure}[H]
  ...
  \caption{Backpropagation computation graph. Black arrows show the forward pass (data flows left to right). Red arrows show the backward pass --- error signals flow right to left, each carrying a gradient term computed by the chain rule. The weight gradient at each layer is the product of the upstream delta with the downstream activations.}
  \label{fig:backprop}
\end{figure}

The key mathematical insight is that the chain rule factorises the total derivative into a product of local derivatives at each layer. Each layer only needs to know the gradient flowing into it from the layer above.
...
\begin{figure}[H]
  ...
  \caption{Left: Fourier approximations with $N = 1, 3, 5, 10$ terms compared to the exact sigmoid. Even 5 terms give a close match. Centre: coefficient magnitudes decay rapidly, confirming the sigmoid is low-frequency dominated. Right: maximum approximation error vs.\ $N$ on a log scale.}
  \label{fig:fourier}
\end{figure}

\subsection{What This Tells Us About the Network}

The rapid decay $|b_1| \gg |b_2| \gg |b_3| \cdots$ tells us the sigmoid is primarily sensitive to slow-varying, low-frequency patterns. High-frequency noise is attenuated naturally. This is why the sigmoid network learns smooth, generalising decision boundaries rather than memorising every noisy fluctuation in the training data.
...
The loss function $\mathcal{L}(\mathbf{W})$ for a neural network is a composition of sigmoid functions and matrix multiplications across multiple layers. Setting $\nabla_{\mathbf{W}}\mathcal{L} = 0$ and solving analytically is not feasible for any non-trivial architecture. Numerical optimisation methods are therefore a necessity.

\subsection{Methods Compared}

\begin{table}[H]
  \centering
  \caption{Comparison of optimisation methods for neural network training}
  \renewcommand{\arraystretch}{1.3}
  \begin{tabular}{lp{3.2cm}ccc}
    \toprule
    \textbf{Method} & \textbf{Information Used} & \textbf{Cost/Step} & \textbf{Convergence} & \textbf{Topic}\\
    \midrule
    Gradient Descent  & $\nabla\mathcal{L}$                        & $O(n)$   & Linear    & Calculus \\
    Newton--Raphson   & $\nabla\mathcal{L}$, $\nabla^2\mathcal{L}$ & $O(n^3)$ & Quadratic & Num.\ Methods \\
    Stochastic GD     & $\nabla\mathcal{L}$ (1 sample)             & $O(1)$   & Noisy     & Statistics \\
    Mini-batch GD     & $\nabla\mathcal{L}$ (batch $b$)            & $O(b)$   & Practical & Lin.\ Algebra \\
    \bottomrule
  \end{tabular}
  \label{tab:methods}
\end{table}
...
To verify the implementation, the algorithm was tested on the one-dimensional loss:
\[
...
\begin{table}[H]
  \centering
  \caption{Gradient descent on $\mathcal{L}(w) = \frac{1}{2}(\sigma(w)-0.8)^2$. Minimum: $w^* = \ln 4 \approx 1.386$.}
  \renewcommand{\arraystretch}{1.3}
  \begin{tabular}{cccc}
    \toprule
...
\begin{keyresult}{Verification}
  Gradient descent converged to $w \approx 1.386 = \ln 4$ within 20 iterations, confirming both the implementation and the theoretical prediction. Newton--Raphson reaches the same tolerance in approximately 5 iterations, at the cost of computing the second derivative at each step.
\end{keyresult}
...
For the same one-dimensional problem, Newton--Raphson converges in approximately 5 iterations versus 20 for gradient descent. The trade-off is that computing and inverting the Hessian $\nabla^2\mathcal{L}$ costs $O(n^3)$ for $n$ parameters --- prohibitively expensive for large networks. Gradient descent at $O(n)$ per step is therefore the practical choice for neural network training.
...
\begin{figure}[H]
  ...
  \caption{Training curves over 700 epochs. Left: cross-entropy loss falls sharply in the first 150 epochs, then plateaus near zero. Right: accuracy rises from approximately 50\% (random at initialisation) to 99.3\%, with the 99\% reference line shown. The behaviour is fully consistent with gradient descent converging to a minimum of $\mathcal{L}$.}
  \label{fig:train}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Decision boundary at four stages of training (red = class 0, blue = class 1). Epoch~0: essentially random (50\%). Epoch~50: early structure emerges. Epoch~200: XOR quadrants become visible. Epoch~699: boundary correctly isolates all clusters at 99\% accuracy. This is gradient descent and backpropagation made tangible.}
  \label{fig:db}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Left: mean weight magnitude grows as the network moves from random initialisation toward a task-specific solution; shaded band shows $\pm1$ std dev. Right: gradient magnitude (log scale) decreases by several orders of magnitude, approaching zero at convergence --- the condition $\nabla\mathcal{L} = 0$ predicted by calculus.}
  \label{fig:wg}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Snapshot of the live training dashboard (\texttt{dashboard.py}) at end of training. The five panels show: decision boundary, training loss, training accuracy, live weight bar chart, and live gradient bar chart. Run \texttt{python dashboard.py} to animate in real time, or \texttt{python dashboard.py --save} to export as MP4/GIF.}
  \label{fig:dashboard}
\end{figure}
...
\begin{enumerate}[leftmargin=2em, itemsep=5pt]
  \item \textbf{Linear Algebra:} Condition number $\kappa \approx 1.73$ confirms a well-conditioned initialisation. SVD shows the network compresses data unevenly but not damagingly, keeping gradient flow stable.

  \item \textbf{Differential Calculus:} Analytical minimum $w^* = \ln 4 \approx 1.386$ was verified against gradient descent convergence within 20 iterations. Backpropagation correctly propagated error signals through all three layers.

  \item \textbf{Fourier Analysis:} The sigmoid is dominated by its first harmonic ($b_1 \approx 0.391$), confirming its low-pass character. Numerical integration error was below $2\times10^{-4}$.

  \item \textbf{Numerical Methods:} Gradient descent converged to \textbf{99.3\% accuracy} in 700 epochs. Gradient magnitude decayed by several orders of magnitude at convergence.
\end{enumerate}
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.2cm} p{4.2cm} p{3.2cm} r}
    \toprule
    \textbf{Topic} & \textbf{Role} & \textbf{Key Result} & \textbf{Value} \\
    \midrule
    Linear Algebra        & Forward pass, weight analysis      & Condition number $\kappa$ & $\approx 1.73$ \\
    Differential Calculus & Backpropagation, minimisation      & Analytical min.\ $w^*$    & $\ln 4 \approx 1.386$ \\
    Fourier Analysis      & Sigmoid frequency structure        & Leading coeff.\ $b_1$     & $\approx 0.391$ \\
...
\begin{itemize}[leftmargin=2em, itemsep=4pt]
  \item Analyse gradient descent as a discrete dynamical system using the ODE stability framework, to formally characterise the convergence radius.
  \item Extend the Fourier analysis to ReLU and Swish activations and compare their frequency profiles to the sigmoid.
  ...
  \item Scale to a deeper network and track how the condition number of each weight matrix evolves during training.
\end{itemize}
...
\[
  \boxed{\lambda_1 \approx 0.892, \qquad \lambda_2 \approx 0.299,
  \qquad \kappa = \sqrt{\lambda_1/\lambda_2} \approx 1.73}
\]
...
\[
  \frac{\sigma(0.001)-\sigma(-0.001)}{0.002}
  = \frac{0.50025 - 0.49975}{0.002} = 0.25 \;\checkmark
\]
...
\[
  ...
  \;\Rightarrow\; e^{-w^*} = 0.25
  \;\Rightarrow\; w^* = -\ln(0.25) = \ln 4 \approx 1.386 \;\checkmark
\]
...
Since the integrand is even (product of the odd part of $\sigma$ with $\sin x$), numerical integration gives:
\[
...
\addcontentsline{toc}{section}{Project Credits}
% ============================================================

\begin{center}
  {\large\bfseries\color{navrachana} CME202 Minor Project --- Group Contributions}\\[0.4cm]
  {\small\color{gray} Navrachana University $\cdot$ School of Engineering and Technology $\cdot$ 2026}
\end{center}
...
\begin{tabular}{>{\bfseries}p{4.5cm} p{9.5cm}}
  \toprule
  \textbf{Contributor} & \textbf{Work Done} \\
  \midrule
  Harshal Vakhariya & All Python code (neural network, training, math plots, live dashboard), all mathematical modelling and derivations, full \LaTeX\ report writing and typesetting, GitHub repository setup and documentation \\
  \bottomrule
\end{tabular}
...
\begin{tabular}{>{\bfseries}p{4.5cm} p{9.5cm}}
  ...
  \midrule
  Utsav Patel   & MOM (Memorandum of Meeting) maintainer --- recorded all group meeting notes and decisions \\[4pt]
  Meet Chauhan  & PPT preparation --- compiled and designed the presentation slides \\[4pt]
  Kathen Kale   & Poster design --- created the project poster for display \\[4pt]
  Aum Nimkar    & Research helper --- assisted with background research and reference collection \\
  \bottomrule
\end{tabular}
...
\begin{center}
  \small\color{gray}
  Full project credits available at:\\
  \url{https://github.com/harshalharshalvakharia-droid/maths-project}
\end{center}
